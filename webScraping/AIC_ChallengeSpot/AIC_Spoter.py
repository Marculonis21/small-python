...
path = "/home/marculonis/Desktop/main-python/webScraping/AIC_ChallengeSpot"

for item in os.listdir(path+"/data"):
    os.remove(path+"/data/{}".format(item))
...
def webScrapeImages():
    ...
    for i in tqdm(range(len(challengeList))):
        ...
        title = endBody.find("span").text
        attribs['org'] = title

        pos = challengeList[i].find('a')
        site = pos.get("href")
        orig = "https://www.aicrowd.com"
        SITE = str(orig+site)
        #print(SITE)
        attribs['site'] = SITE
        ...
        imagePaths.append(path+"/data/"+str(pathX.split("/").pop()))
...
def on_draw():
    ...
    if(ENTERPRESS):
        ENTERPRESS = False
        webbrowser.open_new(str(challengeAttribs[imagePick]["site"]))
